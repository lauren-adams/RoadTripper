...
const EditTrip = () => {
    ...
    const submitHandler = (event) => {
        ...
        if(data.start === ""){
            data.start = userCtx.start;
        }
        if(data.end === ""){
            data.end = userCtx.end;
        }
        if(data.date === ""){
            data.date = userCtx.date;
        }
        if(data.rating === ""){
            data.rating = userCtx.rating;
        }
            ...
            <form className={styles.formLogin} onSubmit={submitHandler} autoComplete="off">
                ...
                <div>
                    <div className={errors.start && touched.start ? styles.unCompleted : !errors.start && touched.start ? styles.completed : undefined}>
                        <input type="text" name="start" value={data.start} placeholder={userCtx.start} onChange={changeHandler} onFocus={focusHandler} autoComplete="off" />
                    </div>
                ...
                <div>
                    <div className={errors.end && touched.end ? styles.unCompleted : !errors.end && touched.end ? styles.completed : undefined}>
                        <input type="text" name="end" value={data.end} placeholder={userCtx.end}  onChange={changeHandler} onFocus={focusHandler} autoComplete="off" />
                    </div>
                ...
                <div>
                    <div className={errors.date && touched.date ? styles.unCompleted : !errors.date && touched.date ? styles.completed : undefined}>
                        <input type="text" name="date" value={data.date} placeholder={userCtx.date}  onChange={changeHandler} onFocus={focusHandler} autoComplete="off" />
                    </div>
                ...
                <div>
                    <div className={errors.rating && touched.rating ? styles.unCompleted : !errors.rating && touched.rating ? styles.completed : undefined}>
                        <input type="text" name="rating" value={data.rating} placeholder={userCtx.rating} onChange={changeHandler} onFocus={focusHandler} autoComplete="off" />
                    </div>
